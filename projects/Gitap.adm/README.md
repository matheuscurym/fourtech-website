...
```text
index.html                  Entrada padrão da hospedagem (redirecionamento)
gitap.html                  Página principal, com o nome solicitado
css/
  gitap.css                 Estilos e responsividade
js/
  gitap.js                  Interações, animações e formulário
assets/
  images/
    logo-gitap-transparente.png
    logo-gitap-clara.png
    logo-original.png
  README.md                 Origem e tratamento das imagens
README.md                   Apresentação e instruções do projeto
THIRD_PARTY_NOTICES.md       Créditos e licença do ícone
```
...
## Por que existem index.html e gitap.html?

`gitap.html` contém a página real e conserva o nome solicitado para este projeto. `index.html` é apenas uma entrada curta que redireciona para ela, porque servidores de hospedagem normalmente procuram esse nome quando alguém acessa a raiz do endereço. Não há duas cópias do conteúdo. Os dois arquivos usam o script em `js/gitap.js`; a entrada executa somente o redirecionamento, preservando a seção e os parâmetros do endereço.
